...
// Import komponen khusus peminjaman
...
export default function Dashboard({ stats, chartData, year, latestMahasiswa, firstAdministrator }: Props) {
    ...
    // 1. Tambahkan state chartKey yang hilang
    ...
    const handleYearChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        router.get(route('item-lending.dashboard'), { year: e.target.value }, { preserveState: true });
    };

    const currentYear = new Date().getFullYear();
    const years = Array.from({ length: 5 }, (_, i) => currentYear - i); // Sesuai peminjaman.blade.php

    // 2. Tambahkan useEffect untuk memperbarui chartKey saat chartData berubah
    React.useEffect(() => {
    ...
    return (
        <AppLayout>
            <Head title="Beranda - Peminjaman" />
            <div className="p-6">
                <header className="mb-6">
                    <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Dashboard Peminjaman</h2>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Ringkasan aktivitas peminjaman barang.</p>
                </header>

                {/* Grid Statistik - Dinamis berdasarkan Role */}
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 w-full">
                    {auth.user?.role === 'mahasiswa' ? (
                        <>
                            ...
                            <StatCard
                                color="red"
                                label="Jatuh Tempo Kembali"
                                value={stats.jatuhTempoKembali ?? '-'}
                            />
                        </>
                    ) : (
                        <>
                            {/* Tampilan Admin (Sesuai Layout Awal Anda) */}
                            <StatCard
                                color="blue"
                ...
                {/* Kontainer Utama - Menggunakan min-w-0 w-full agar tidak bergeser */}
